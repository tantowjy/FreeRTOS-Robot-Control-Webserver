...
void ServerTask(void * pvParameters) {
  ...
  for (;;) {
    server.handleClient();
    delay(1);  // Yielding to other tasks
  }
}
...
void setup() {
  ...
  xTaskCreatePinnedToCore(WebSocketTask, "WebSocketTask", 8192, NULL, 1, NULL, 1);
}

void loop() {
  // No need to put code here as tasks are already running.
}
